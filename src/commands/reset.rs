...
impl SlashCommand for ResetCommand {
    ...
    async fn execute(
        ...
        // 3. 對 Pi backend：清空 session 檔案內容（保留檔案本身）
        if agent_type.to_string() == "pi" {
            let session_file = migrate::get_sessions_dir("pi")
                .join(format!("discord-rs-{}.jsonl", channel_id_u64));
            if session_file.exists() {
                if let Err(e) = tokio::fs::write(&session_file, "").await {
                    let i18n = state.i18n.read().await;
                    let msg = format!("{}: {}", i18n.get("reset_truncate_failed"), e);
                    drop(i18n);
                    command
        ...
        // 5. 重新建立 session（Pi 會讀取已清空的 JSONL；OpenCode/Kilo 會建立新 HTTP session）
        match state
            .session_manager
            ...
            .await
        {
            Ok(_) => {
                let i18n = state.i18n.read().await;
                let msg = i18n.get("reset_success");
                drop(i18n);
                command
                    .edit_response(&ctx.http, EditInteractionResponse::new().content(msg))
                    .await?;
            ...
            Err(e) => {
                let i18n = state.i18n.read().await;
                let msg = format!("{}: {}", i18n.get("reset_failed"), e);
                drop(i18n);
                command
                    .edit_response(&ctx.http, EditInteractionResponse::new().content(msg))
                    .await?;
